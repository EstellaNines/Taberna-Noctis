...
namespace TabernaNoctis.EditorTools
{
    public sealed class RandomMonitorWindow : EditorWindow
    {
        ...
        public void CreateGUI()
        {
            ...
            // 引入全局颜色与组件样式（优先 Editor/，兼容旧路径）
            root.styleSheets.Add(LoadStyle("Assets/Scripts/Editor/UITK/EditorColors.uss", "Assets/Scripts/0_Editor/UITK/EditorColors.uss"));
            root.styleSheets.Add(LoadStyle("Assets/Scripts/Editor/UITK/RandomMonitorWindow.uss", "Assets/Scripts/0_Editor/UITK/RandomMonitorWindow.uss"));
        ...
        private static StyleSheet LoadStyle(string preferred, string fallback)
        {
            // 根据传入的文件名拼接 Editor 路径优先
            string fileName = Path.GetFileName(preferred);
            string editorPath = "Assets/Editor/UITK/" + fileName;
            var e = AssetDatabase.LoadAssetAtPath<StyleSheet>(editorPath);
            if (e != null) return e;
            var ss = AssetDatabase.LoadAssetAtPath<StyleSheet>(preferred);
            if (ss != null) return ss;
            return AssetDatabase.LoadAssetAtPath<StyleSheet>(fallback);
        }
