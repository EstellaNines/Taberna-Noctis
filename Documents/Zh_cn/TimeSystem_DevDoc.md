...
时间系统提供从早上、下午到夜晚的整日阶段编排与真实/游戏时间换算，自动推进并处理跨天，通过消息驱动场景切换与存档钩子（早 → 晚、晚 → 结算、新一天开始），可由脚本化配置控制阶段时长、倍率和起止时刻，并支持在 Loading/结算等流程中受控暂停与恢复，构成全局时序与场景流转的中枢。
...
### Phase state machine diagram(阶段状态机图)A
...
## 7. Message System integration（消息系统集成）
...
#### 3）Time multiplier and clock conversion(时间倍率与时钟换算)
