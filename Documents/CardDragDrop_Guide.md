...
卡牌拖拽系统为 `CardSlot` 提供即插即用的拖放交互，围绕开始/拖拽/结束三态与统一事件契约（OnCardSlotDropped、ConfirmDropHandled）解耦交互与业务逻辑；配合发牌队列、存储与提交区域无缝协作，未命中时自动回位，命中时按区域策略消费或入库，并配套时序/ER/类图以支撑快速集成与排错。
