...
- 状态基线占比（stateBasePercent）：五种状态（忙碌/急躁/烦闷/挑剔/友好）在当日的基线占比各为 20%，合计 100%。
- NPC 个体占比（visitPercent）：在不考虑当日事件时，所有 NPC 在全局的基线到访占比均匀分配为 100% / NPC 总数。
- 每日消息调整：当日事件对某一“身份+状态（可含性别或具体 NPC）”的占比做加法调整（deltaPercent，允许正负），在落地计算时以该状态的基线 20% 为参照，即：最终占比 = max(0, 20% + deltaPercent)。
- 说明：如需更精细的全局归一化，可在实现层对所有状态/身份/个体的最终值做一次归一化处理以确保合计为 100%。
