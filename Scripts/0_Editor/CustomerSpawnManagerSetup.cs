...
public class CustomerSpawnManagerSetup
{
    ...
    public static void SetupReferences()
    {
        ...
        if (characterRolesIndexField != null && rolesIndex != null)
        {
            ...
            hasChanges = true;
            Debug.Log("✅ 已设置 CharacterRolesIndex 引用");
        }
        ...
        if (npcDatabaseField != null && npcDatabase != null)
        {
            ...
            hasChanges = true;
            Debug.Log("✅ 已设置 NpcDatabase 引用");
        }

        if (hasChanges)
        {
            EditorUtility.SetDirty(spawnManager);
            Debug.Log("✅ 引用设置完成！请保存场景");
        }
        else
        {
            Debug.LogWarning("❌ 未找到可设置的引用");
        }
    ...
    private static void VerifySetup(CustomerSpawnManager spawnManager)
    {
        ...
        if (rolesIndex != null)
        {
            Debug.Log($"✅ CharacterRolesIndex: {rolesIndex.name}");
        ...
        {
            Debug.LogWarning("❌ CharacterRolesIndex 未设置");
        }

        if (database != null)
        {
            Debug.Log($"✅ NpcDatabase: {database.name}");
            Debug.Log($"   - NPC数量: {database.allNpcs.Count}");
        }
        else
        {
            Debug.LogWarning("❌ NpcDatabase 未设置");
        }

        if (rolesIndex == null && database == null)
        {
            Debug.LogError("❌ 至少需要设置 CharacterRolesIndex 或 NpcDatabase 中的一个！");
    ...
    public static void CreateCustomerSpawnManagerGameObject()
    {
        ...
        Debug.Log("✅ 已创建 CustomerSpawnManager GameObject");
